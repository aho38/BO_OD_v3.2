...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    conf_ls = [torch.zeros((0,80), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            conf = x[i, 5:]
            x = torch.cat((box[i], x[i, 4:5], x[i, j + 5, None], j[:, None].float()), 1)
        ...
        if not n:  # no boxes
            ...
            conf = conf[x[:, 4].argsort(descending=True)[:max_nms]]
